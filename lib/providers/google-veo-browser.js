...
async function generate(o = {}) {
  ...
  try {
    ...
    const loggedIn = await page.locator('text=/new project|create|prompt|generate|new scene|scenebuilder|video/i').first().isVisible({ timeout: 8000 }).catch(() => false);
    ...
    // fill the VISIBLE prompt textarea (exclude the hidden g-recaptcha-response textarea — that hidden one is why the first attempt timed out).
    const promptSel = process.env.FLOW_PROMPT_SELECTOR || '[role="textbox"][contenteditable="true"], textarea:not(.g-recaptcha-response)';
    ...
    if (!srcUrl) { await closeCtx(); return { ok: false, provider, reason: 'flow_result_timeout' }; }
