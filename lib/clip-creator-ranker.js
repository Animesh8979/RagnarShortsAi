...
function rankCreators(opts = {}) {
  ...
  const pw = cfg.performanceWeight ?? 0.6;
  const tw = cfg.trendingWeight ?? 0.4;
...
if (require.main === module) {
  ...
  for (const r of ranked) {
    console.log(`  ${String(r.combined).padStart(3)}  ${r.creator.padEnd(16)} perf=${r.perf}${r.perfColdStart ? '(cold)' : '(' + r.perfN + ' clips)'} trending=${r.trending}`);
  }
}
