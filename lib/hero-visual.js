...
async function heroVisual(input) {
  ...
  // Beat metadata may include `totalBeats` so the climax index is known.
  // Falls through to Tier-1 parallax if all i2v providers are queued/down.
  ...
  try {
    ...
    if (i2v.shouldUseHeroI2v(beatIndex || 0, totalBeats)) {
      ...
      // Log + continue to Tier-1.
      try { console.log(`   ⚠  i2v failed (${(r2.reason || '').slice(0, 80)}) — falling to Tier-1 parallax`); } catch (_) {}
    }
