...
const VOICE = process.env.KOKORO_VOICE || 'am_michael'; // energetic male; af_heart for female
...
async function synthesize(opts) {
  ...
  try {
    ...
    const audio = await tts.generate(text, { voice: opts.voice || VOICE });
  ...
  return { ok: true, audioPath: mp3Path, durationSec, wordBoundaries, voice: opts.voice || VOICE, engine: 'kokoro-82m' };
}
