...
const ROUTES = {
  ...
  // L116 B2: google-veo-flow FIRST (Veo 3.1 via the user's Flow session = the cinematic
  // 95 lever) — only reached on hero beats (shouldUseHeroI2v), self-gates on budget/session,
  // cascades to nvidia-cosmos/HF when disabled or exhausted (zero regression).
  motion_video:      ['google-veo-flow', 'nvidia-cosmos', 'hf-ltx-video', 'hf-wan-2.1', 'hf-cogvideox', 'fal-ltx'],
...
const KEY_REQUIREMENTS = {
  ...
  'google-veo-flow':           ['GOOGLE_FLOW_SESSION'], // L116 B2: skipped unless the Flow flag is set (no wasted attempt when disabled)
