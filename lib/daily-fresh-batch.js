...
function parseArgs() {
  ...
  return {
    ...
    uploadGapMin: Math.max(0, Number(flag('upload-gap-min', 120)) || 0),
...
async function runOrganicLane(opts) {
  ...
  const trending = await fetchTrending({ topN: Math.max(opts.organicN * 4, 8) });
  ...
  for (let i = 0; i < accepted.length; i++) {
    ...
    try {
      // +15% lands at ~140 wpm — natural newscast pace. Captions sync via
      // Edge TTS word boundaries (slower rate → wider boundary spacing →
      // captions auto-stretch). Drop further to +10% if still too fast.
      const r = await processScript(a.script.scriptId, a.file, '+15%', outDir);
...
async function runClipsLane(opts) {
  ...
  for (const c of candidates.slice(0, opts.clipsN)) {
    ...
    const moments = pickMoments(cutDurationSec, opts.momentsPerClip);
