  ...
  if (opts.autoUpload && !opts.dryRun) {
    ...
    if (process.env.UPLOAD_VIA_QUEUE === '1') {
      ...
      try {
        ...
        let n = 0;
        for (const r of organic.filter((x) => x.ok)) { queue.enqueue({ render: r, kind: 'organic' }); n++; }
        for (const r of clips.filter((x) => x.ok)) { queue.enqueue({ render: r, kind: 'clip' }); n++; }
        const { spawn } = require('child_process');
        const d = spawn(process.execPath, [path.join(ROOT, 'tools', 'upload-daemon.js')], { detached: true, stdio: 'ignore' });
        d.unref();
