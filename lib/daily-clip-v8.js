...
async function processClip(clip) {
  ...
  //    L109 P1 — when L109_CUT_ON_PEAK=1 AND clip.mode==='full_frame_horror',
  //    use the peak-detector + multi-cut + SFX overlay path instead of a
  //    single-shot crop. This is the cheat-code that takes us from 1 cut
  //    per 28s to 8-14 cuts per 28s, with whoosh on every cut + vine-boom
  //    on top-3 peaks.
  const arollPath = path.join(clip.outDir, slug + '-aroll.mp4');
  let arollMethod = 'single-shot';
  let cutAndStackResult = null;
  if (process.env.L109_CUT_ON_PEAK === '1' && clip.mode === 'full_frame_horror') {
    try {
      ...
      if (widR.status === 0 && fs.existsSync(widePath)) {
        ...
        if (cutAndStackResult.ok) {
          arollMethod = `cut-and-stack (${cutAndStackResult.cuts.length} cuts, ${cutAndStackResult.sfxOverlays.length} SFX, ${cutAndStackResult.peakDetectorMs}ms peak + ${cutAndStackResult.renderMs}ms render)`;
