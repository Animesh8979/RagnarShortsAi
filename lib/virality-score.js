...
async function callGemini(prompt) {
  // gemini-1.5-flash was retired by Google (404). Use the shared env default
  // (gemini-2.5-flash, set in env-d-drive-only) so this fallback actually works.
  ...
  if (!r.ok) throw new Error('gemini_http_' + r.status + ': ' + (await r.text()).slice(0, 200));
