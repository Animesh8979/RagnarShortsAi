...
function buildExplodedChunk(chunkWords, chunkStart, chunkEnd, powerWordSet, y = DEFAULT_CAPTION_Y) {
  const lines = [];
  const baseText = chunkWords.map((c) => escapeAss(c.word)).join(' ');
  // Background base layer: all-white chunk text shown the entire chunk duration.
  lines.push(`Dialogue: 0,${fmtAssTime(chunkStart)},${fmtAssTime(chunkEnd)},Default,,0,0,0,,{\\pos(540,${y})}${baseText}`);
  // Per-word overlay layer: each word "explodes" during its active window.
  for (let wi = 0; wi < chunkWords.length; wi++) {
    const w = chunkWords[wi];
    const wStart = w.startSeconds;
    const wEnd = w.startSeconds + Math.max(0.05, w.durationSeconds);
    const isPower = isPowerWord(w.word, powerWordSet);
    // Inline tags: \c color (BGR), \fs size, \bord outline, \blur glow, \fscx/\fscy scale 110%
    const colorTag = isPower ? '\\c&H0000D7FF&' : '\\c&H00FFFFFF&';
    const sizeTag = isPower ? '\\fs108\\fscx112\\fscy112' : '\\fs96\\fscx108\\fscy108';
    const glowTag = isPower ? '\\bord10\\3c&H00000000&\\blur1.5' : '\\bord8\\3c&H00000000&\\blur0.8';
    ...
    const middle = `{${colorTag}${sizeTag}${glowTag}}${escapeAss(w.word)}{\\r}`;
    // Use Layer 1 so it stacks ABOVE the base white text from above.
    const eventText = [before, middle, after].filter(Boolean).join(' ');
    lines.push(`Dialogue: 1,${fmtAssTime(wStart)},${fmtAssTime(wEnd)},Default,,0,0,0,,{\\pos(540,${y})}${eventText}`);
  }
  return lines;
}
