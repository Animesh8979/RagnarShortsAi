...
async function cutAndStack(opts) {
  ...
  const cuts = planCuts(peaks, targetDurSec, 8, 14);
  if (cuts.length < 4) return { ok: false, reason: `only ${cuts.length} cuts planned`, peakDetectorMs: peakMs };

  // Build SFX overlays. Each cut boundary (output-time) gets whoosh.
  // Top-3 cuts get vine-boom layered on top.
  const overlays = [];
  let outTime = 0;
  for (let i = 0; i < cuts.length; i++) {
    if (i > 0) overlays.push({ t: outTime, category: 'whoosh', gainDb: -6 });
    if (cuts[i].gotBoom) overlays.push({ t: outTime + 0.05, category: 'vine-boom', gainDb: -3 });
    outTime += cuts[i].segDur;
  }
...
module.exports = { cutAndStack, planCuts, buildConcatChain };
