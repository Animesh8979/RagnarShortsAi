...
async function callHfInferenceI2V({ modelId, imagePath, prompt, durationSec, expectedFps = 16 }) {
  ...
  const cacheKey = sha1(`${modelId}|${imagePath}|${prompt}|${durationSec}`);
  ...
  if (fs.existsSync(outPath) && fs.statSync(outPath).size > 200_000) {
    return { ok: true, path: outPath, provider: modelId, cached: true };
  }
...
async function generateHeroBeat({ imagePath, motionPrompt, durationSec }) {
  ...
  const result = await router.withFailover('motion_video', async (provider) => {
    ...
    if (provider === 'hf-ltx-video') {
      const r = await callHfInferenceI2V({ modelId: 'Lightricks/LTX-Video', imagePath, prompt: motionPrompt, durationSec, expectedFps: 25 });
