...
const MODEL = process.env.AESTHETIC_QA_MODEL || 'gemini-2.5-flash';
...
async function callGeminiVision(frameBase64) {
  ...
  if (!apiKey) return { ok: false, reason: 'no_gemini_api_key' };
  ...
  } catch (err) { return { ok: false, reason: `vision_fetch_failed: ${err && err.message || err}` }; }
  if (!response.ok) {
    const body = await response.text().catch(() => '');
    return { ok: false, reason: `vision_http_${response.status}: ${body.slice(0, 200)}` };
  }
  let data; try { data = await response.json(); } catch (_) { return { ok: false, reason: 'vision_response_not_json' }; }
  const text = data && data.candidates && data.candidates[0] && data.candidates[0].content
    ? data.candidates[0].content.parts.map((p) => p.text || '').join('')
    : '';
  if (!text) return { ok: false, reason: 'vision_empty_text' };
  return { ok: true, text };
}
