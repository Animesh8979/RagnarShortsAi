  ...
  if (skipYt) {
    ...
    try {
      ...
      try {
        ...
        if (!isOrganic && render.spec && /metadata_too_similar|invalid video description|invalidDescription/i.test(msg)) {
          ...
          try {
            ...
            if (regen.ok) {
              console.log(`  YT: regen OK (${regen.type}) → "${regen.title}"; retrying...`);
...
async function run(opts) {
  ...
  const laneOpts = { startAt, dryRun: o.dryRun, gapMin: o.gapMin, skipYtOnFirst: o.skipYtOnFirst, initialDelayMin: o.initialDelayMin || 0 };
