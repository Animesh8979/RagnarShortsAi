...
function buildClipMetadata(render) {
  ...
  // Title: prefix with a moment-cue so two clips from different sources
  // never collide on title even with the same source-title token soup.
  const titlePrefixes = [
    `${creator}'s ${momentKeyword} moment`,
    `Inside ${creator}'s ${sourceTitle}`,
    `${creator}: ${sourceTitle.split(/[—\-:]/)[0].trim().slice(0, 60)}`,
    `${sourceTitle.slice(0, 80)} — ${creator}`,
  ];
...
async function uploadOne({ render, kind, dryRun, log, skipYt = false, skipIg = false }) {
  ...
  if (isOrganic) {
    console.log('  IG: skipped (organic content is skipped on Instagram per user settings)');
    item.instagram = { success: true, skipped: true, note: 'Skipped organic on Instagram' };
  } else {
    try {
...
async function run(opts) {
  ...
  for (let i = startAt; i < queue.length; i++) {
    ...
    // Phase 1.4 prep — record to performance-ledger so refresh-youtube-analytics
    // + channel-cluster-router can find this video in future analytic runs.
    try {
      ...
      if (last && last.youtube && last.youtube.success && last.youtube.videoId) {
        ...
        recordPerformanceEntry({
          ...
          result: {
            ...
            uploadedInstagram: !!(last.instagram && last.instagram.success),
          },
          metadata: { title: last.title, tags: [] },
          uploadResult: {
            success: true,
            mediaId: last.youtube.videoId,
            channelLabel: last.kind === 'clip' ? 'RagnarShortsUltimate' : 'RagnarShortsAi',
            channel: last.kind === 'clip' ? 'RagnarShortsUltimate' : 'RagnarShortsAi',
            url: last.youtube.videoUrl,
            permalink: last.youtube.videoUrl,
          },
          instagramResult: last.instagram && last.instagram.success ? {
            success: true,
            mediaId: last.instagram.mediaId,
            url: last.instagram.permalink,
            permalink: last.instagram.permalink,
          } : null,
        });
      }
    } catch (e) { /* non-fatal */ }
