...
// ── Upload one item ──────────────────────────────────────────────────────
async function uploadOne({ render, kind, dryRun, log }) {
...
async function run(opts) {
  ...
  console.log(`=== AUTO-UPLOAD FRESH BATCH ${o.date} ===  items=${queue.length}  gap=${o.gapMin}m  dryRun=${o.dryRun}`);
  const log = { ranAt: new Date().toISOString(), date: o.date, gapMin: o.gapMin, items: [] };

  for (let i = 0; i < queue.length; i++) {
    await uploadOne({ ...queue[i], dryRun: o.dryRun, log });
