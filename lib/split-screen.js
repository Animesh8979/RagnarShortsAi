...
function assessSourceQuality(videoPath, options = {}) {
  ...
  const minDurationSec = Number(options.minDurationSec || 8 * 60);
...
async function compose(opts) {
  ...
  if (mode === 'full_frame_horror') {
    // ── FULL-FRAME HORROR mode (no b-roll) ──────────────────────────────
    // For IShowSpeed horror VODs and similar: vertical 9:16 center-crop of
    // the gameplay, brain-rot grade, reactive captions burned over bottom
    // 25% with a dim scrim for legibility.
    // Why no b-roll: jumpscare cadence + facecam expression are the entire
    // payload — Subway Surfers below kills the dread.
    filterComplex = [
      // 1. Vertical center-crop (scale-up so 16:9 fills 1080×1920, crop center).
      `[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setsar=1[v_raw]`,
      // 2. Adaptive EQ + unsharp (horror sources tend to be dark; lift them).
      `[v_raw]${eq},unsharp=5:5:0.8:5:5:0.4[v_eq]`,
      // 3. Bottom 25% dim scrim so reactive captions read clean.
      `color=c=black@0.40:s=1080x480:d=${durationSec.toFixed(2)}[caption_scrim]`,
      `[v_eq][caption_scrim]overlay=0:1440[v_scrim]`,
    ...
    filterComplex = [
      ...
      `[0:v]scale=1080:960:force_original_aspect_ratio=increase,crop=1080:960,gblur=sigma=22,setsar=1[abg]`,
      // A-roll foreground (full frame visible, no faces cropped)
      `[0:v]scale=1080:960:force_original_aspect_ratio=decrease,setsar=1[afg_raw]`,
      ...
      // B-roll untouched (just crop-fit)
      `[1:v]scale=1080:960:force_original_aspect_ratio=increase,crop=1080:960,setsar=1[b]`,
