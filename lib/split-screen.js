...
async function compose(opts) {
  ...
  // A-roll pipeline: blurred-fill background (zoom-crop) + foreground (scale-fit) +
  // adaptive EQ + unsharp. ALL applied only to [0:v] (the source A-roll).
  // B-roll [1:v] is scale-cropped without enhancement.
  const filterComplex = [
    ...
    `[v0]fps=30${captionsFilter}[vfinal]`,
  ].join(';');
  ...
  if (r.status !== 0 || !fs.existsSync(outputPath) || fs.statSync(outputPath).size < 100_000) {
    return { ok: false, reason: 'ffmpeg_failed', stderr: r.stderr ? r.stderr.slice(-600) : '', brollUsed: brollFile, brollOffset };
  }

  appendHistory({ file: brollFile, offset: brollOffset, usedAt: new Date().toISOString() });
  return { ok: true, outputPath, brollUsed: brollFile, brollOffset };
}
