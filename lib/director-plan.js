...
function buildDirectorPlan(script, props = {}) {
  ...
  const planMode = props.planMode || (process.env.ORGANIC_DOMINATION === '1' ? 'domination' : 'story');
  ...
  const planBeats = beats.map((beat, index) => {
    ...
    let module = planMode === 'domination' 
      ? 'cinematic' 
      : chooseModule({ topicClass, purpose, beatText: `${beatText} ${visualPrompt}`, index });
    ...
    const emotion = beat.emotion || (purpose === 'human_or_market_cost' ? 'cost' : purpose === 'turn' ? 'tension' : purpose === 'payoff' ? 'uncertainty' : 'urgency');
    ...
    const baseBeat = {
      ...
      // OP-16: Director Intelligence Upgrade - Animation Toolkit Routing
      textMode: purpose === 'hook' || purpose === 'payoff' ? 'hormozi' : (index % 3 === 0 ? 'impact-slam' : 'default'),
      characterStyle: purpose === 'turn' ? 'stickman' : 'vector', 
      transitionType: purpose === 'hook' ? 'glitch' : 'swipe',
      dataVizMode: nums.length > 1 ? (index % 2 === 0 ? 'bar-chart' : 'line-graph') : null,
    };

    if (planMode === 'domination') {
      baseBeat.shotIntent = beat.shotIntent || `Viewer should feel the deep ${emotion} of the scene.`;
      baseBeat.hookStrength = index === 0 ? (beat.hookStrength || "Aggressive visual hook showing narrative tension.") : null;
      baseBeat.retentionTrigger = beat.retentionTrigger || (index === 0 ? 'curiosity_gap' : (index === total - 1 ? 'subscribe_hook' : (index % 2 === 1 ? 'escalation' : 'reveal')));
    }

    return baseBeat;
  });

  const variedBeats = planMode === 'domination' 
    ? planBeats 
    : assignPresenter(enforceVisualVariety(planBeats, topicClass));
...
function auditDirectorPlan(plan) {
  ...
  const isDomination = plan && plan.planMode === 'domination';
  ...
  if (!isDomination && variety < 0.66) reasons.push(`module variety too low ${variety.toFixed(2)}`);
  
  const score = Math.max(0, Math.min(100, Math.round(100 - (isDomination ? reasons.length * 20 : reasons.length * 12 + variety * 10))));
  return {
