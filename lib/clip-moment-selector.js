...
async function scoreWindows(windows) {
  ...
  const prompt = `You are a viral-clip judge for a brain-rot COMEDY clip channel built on FUNNY moments from big streamers and comedians (IShowSpeed, Kai Cenat, Kill Tony, Theo Von, Bobby Lee, Shane Gillis). For each numbered transcript window return TWO scores 0-100:
  "viral": short-form virality — hook, emotional spike, quotable line, payoff/punchline, conflict/stakes.
  "humor": how FUNNY / laugh-out-loud it is — jokes, roasts, absurd reactions, comedic timing, punchlines, the room cracking up. A genuinely hilarious moment = 90+; a flat / serious / context-only window = low.
Funny moments are this channel's priority — reward them hardest.

WINDOWS:
${windows.map((w) => `[${w.i}] ${String(w.text || '').slice(0, 320)}`).join('\n')}

Return STRICT JSON: { "scores": { "0": {"viral":N,"humor":N}, "1": {"viral":N,"humor":N} } }`;
  ...
  async function gemini() {
    const g = await require('./gemini-call').geminiGenerate({ text: prompt, json: true, temperature: 0.2 });
...
async function selectMoments(opts) {
  ...
  const humorMode = process.env.CLIP_HUMOR_TARGETING !== '0'; // funny streamer moments hit big
  for (const w of windows) {
    const s = scores[String(w.i)];
    let viral = 40, humor = 40;
    if (s && typeof s === 'object') { viral = Number(s.viral); humor = Number(s.humor); }
    else if (Number.isFinite(Number(s))) { viral = humor = Number(s); } // backward-compat (flat score)
    if (!Number.isFinite(viral)) viral = 40;
    if (!Number.isFinite(humor)) humor = 40;
    // Humor-weighted when targeting funny (user: funny high-end-streamer clips hit big).
    w.viral = viral; w.humor = humor;
    w.score = humorMode ? Math.round(0.45 * viral + 0.55 * humor) : viral;
  ...
  return {
    ok: true,
    moments: picked.map((w) => ({ startSec: Math.round(w.startSec * 100) / 100, durationSec: windowSec, score: w.score, humor: w.humor, viral: w.viral, intensity_db: w.intensity_db, text: w.text.slice(0, 120) })),
  };
}
