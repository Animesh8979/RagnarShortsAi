...
async function cutAndStack(opts) {
  ...
  const t0 = Date.now();
  const isChaosMode = opts.mode === 'full_frame_horror' || opts.isHorror;
  // Football/reaction/commentary clips have steady-state audio with few transient peaks.
  // For non-horror content, use a more lenient silence threshold (-35dB instead of -25dB)
  // so moderate-level audio transitions are detected as "loud regions" and yield cuts.
  const adaptiveThresholdDb = Number(opts.thresholdDb ?? (isChaosMode ? -25 : -35));
  let peaks = await peakDetector.detectPeaks({
    inputPath: sourcePath,
    thresholdDb: adaptiveThresholdDb,
    minSilenceSec: 0.15,
    minPeakIntervalSec: 1.5,
    maxPeaks: 24,
  });
  // If still insufficient peaks, retry with an even lower threshold (-45dB) to capture
  // quiet but present audio-level changes in commentary/reaction content.
  if (peaks.length < 2 && !isChaosMode) {
    peaks = await peakDetector.detectPeaks({
      inputPath: sourcePath,
      thresholdDb: -45,
      minSilenceSec: 0.15,
      minPeakIntervalSec: 2.0,
      maxPeaks: 24,
    });
  }
  const peakMs = Date.now() - t0;
  const minPeaksRequired = isChaosMode ? 4 : 2;
  if (peaks.length < minPeaksRequired) {
    // Librosa onset detection fallback for steady-audio content (football, reaction)
    const librosaPeaks = beatDetector.detectOnsets(sourcePath);
    if (librosaPeaks.length >= minPeaksRequired) {
      peaks = librosaPeaks.map(t => ({ t, intensity_db: -10, type: 'librosa_onset' }));
    } else {
      // Fixed-interval fallback: cut every 4-5s so clips still have visual rhythm
      const FFPROBE = (() => { try { return require('ffprobe-static').path; } catch (_) { return 'ffprobe'; } })();
      const dur = (() => {
        const r = spawnSync(FFPROBE, ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=nw=1:nk=1', sourcePath], { encoding: 'utf8' });
        return Number((r.stdout || '').trim()) || targetDurSec * 2;
      })();
      const interval = isChaosMode ? 3 : 4.5;
      peaks = [];
      for (let t = interval; t < dur - 1; t += interval) {
        peaks.push({ t, intensity_db: -15, type: 'fixed_interval' });
      }
      if (peaks.length < 2) {
        return { ok: false, reason: `only ${peaks.length} peaks after all fallbacks`, peakDetectorMs: peakMs };
      }
    }
  }
  ...
  const minCuts = Number(process.env.CLIP_MIN_CUTS || (isChaosMode ? 8 : 4));
  const maxCuts = Number(process.env.CLIP_MAX_CUTS || (isChaosMode ? 14 : 6));
