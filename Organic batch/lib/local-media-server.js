...
function createServer({ port } = {}) {
  ...
  const server = http.createServer((req, res) => {
    console.log(`[local-media-server] ${req.method} ${req.url} - range: ${req.headers.range || 'none'} - UA: ${req.headers['user-agent'] || 'none'}`);
    const originalEnd = res.end;
    res.end = function(...args) {
      console.log(`[local-media-server] Response Status: ${res.statusCode} for ${req.url}`);
