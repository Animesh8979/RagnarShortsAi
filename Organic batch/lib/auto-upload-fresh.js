...
async function uploadOne({ render, kind, dryRun, log, skipYt = false, skipIg = false }) {
  ...
  // V11 Domination (ORGANIC_DOMINATION=1) renders a self-contained final MP4 — no sidecar needed.
  const dominationIsPrimary = process.env.ORGANIC_DOMINATION === '1';
  const preferOrganicMotion = isOrganic && process.env.ORGANIC_UPLOAD_PREFER_MOTION !== '0' && !dominationIsPrimary;
  ...
  const requireOrganicMotion = isOrganic && process.env.ORGANIC_UPLOAD_REQUIRE_MOTION !== '0' && !v9IsPrimary && !dominationIsPrimary;
  ...
  // CRITICAL GUARDRAIL: Prevent catastrophic cross-lane uploading
  // L120 — FIFA football CLIPS (IShowSpeed/MrBeast) are LEGITIMATE clip-lane content
  // the user ships to @ragnarautomated on purpose (06-08 FIFA clips are live there).
  // Only block true ORGANIC premium topics leaking onto the clip lane — gate on the
  // worldnews/geopolitics TAGS (which never appear on real creator clips), NOT on
  // 'fifa' or a "World Cup" title keyword (clip titles legitimately say that).
  const isPremiumContent = (meta.tags || []).some(t => ['worldnews', 'geopolitics'].includes(t));
  if (isPremiumContent && !isOrganic) {
    item.error = 'lane_violation_premium_on_clips';
    console.log(`\n=== FATAL OVERRIDE — ${meta.title} ===`);
    console.log('  BLOCKED: Premium/FIFA content attempted to upload to Funny Clips lane (@ragnarautomated). Aborting to protect brand taxonomy.');
    return;
  }
      ...
      try {
        ...
        if (isOrganic && /metadata_too_similar|invalid video description|invalidDescription/i.test(msg)) {
          ...
          try {
            ...
            // L116 — mirror the clip fallback: once we've regenerated DISTINCT organic
            // metadata, skip the re-check gate so an organic is NEVER permanently
            // unuploadable. The geopolitics niche-lock makes war headlines collide
            // (title_too_similar score=1 vs another recent strike/sanctions title);
            // a distinct-but-adjacent title beats the video never going live.
            item.youtube = await uploadToYouTube(videoPath, ytMeta.title, ytMeta.description, ytMeta.tags, { ...ytOpts, skipMetadataUniqueGate: true });
          } catch (regenErr) {
  ...
  if (skipIg) {
    ...
    try {
      ...
      // L116 — HARD dup guard: never re-post the same video to IG regardless of
      // queue/retry state (root-cause fix for "same video uploaded again and again").
      const _sentFile = path.join(ROOT, 'renders', 'queue', 'sent-uploads.json');
      const _sentKey = 'ig:' + path.basename(String(videoPath || ''));
      let _sent = []; try { _sent = JSON.parse(fs.readFileSync(_sentFile, 'utf8')); } catch (_) {}
      if (_sent.includes(_sentKey)) {
        item.instagram = { skipped: true, alreadySent: true };
        console.log('  IG: SKIPPED — this video was already posted to IG (dup guard)');
      } else {
        ...
        if (item.instagram && item.instagram.success) { try { _sent.push(_sentKey); fs.writeFileSync(_sentFile, JSON.stringify(_sent)); } catch (_) {} }
      }
    } catch (e) {
  ...
  // L116 — OFF by default (user: "no carousel usage"). Its slide_*_hero_failed
  // throws were corrupting the IG reel result → job marked not-done → daemon
  // retried → SAME reel re-posted (the duplicate-upload bug). Opt in with CAROUSEL_ENABLED=1.
  if (isOrganic && process.env.CAROUSEL_ENABLED === '1' && item.instagram && item.instagram.success && !item.dryRun) {
    try {
...
module.exports = { run, uploadOne }; // L116 A1: uploadOne exported for the upload-queue daemon
