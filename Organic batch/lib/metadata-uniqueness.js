...
function firstSentence(s) {
  // Split on punctuation in the RAW string first — norm() strips . ! ?, so
  // running norm before the regex collapses the description into one long
  // run and produces false-positive "too similar" matches when only the
  // boilerplate after the first sentence is shared.
  const raw = String(s || '').trim();
  const m = raw.match(/[^.!?\n]{6,}?[.!?\n]/);
  const sentence = m ? m[0] : raw.slice(0, 280);
  return norm(sentence).slice(0, 280);
}
