...
async function buildFromWhisper(input) {
  ...
  // L110 T3.1 — local faster-whisper first (offline, no Groq timeout/dropout
  // — this is the engine that fixes the chronic clip-caption failures). Falls
  // through to Groq API below if Python/model unavailable.
    ...
    try {
      ...
      if (local.isAvailable()) {
        ...
        if (lr.ok && lr.wordBoundaries && lr.wordBoundaries.length) {
          return buildFromBoundaries({ boundaries: lr.wordBoundaries, powerWords, outputPath, mode: input.mode, captionY: input.captionY });
        }
      }
