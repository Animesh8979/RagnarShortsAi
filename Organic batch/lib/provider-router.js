...
const ROUTES = {
  ...
  // pollinations-seedream removed (500 errors), hf-flux-schnell removed (402 credits depleted 2026-06-13)
  hero_image:        ['nvidia-flux.1-dev', 'nvidia-flux.2-klein', 'gemini-image'],
  // motion_video: Veo 3.1 (browser, free 95/mo) → fal-ltx (paid budget-gated fallback).
  // DEAD PROVIDERS REMOVED 2026-06-13: nvidia-cosmos (404), hf-ltx-video (deprecated),
  // hf-wan-2.1 (deprecated), hf-cogvideox (deprecated) — all 84 failures each, wasting retries.
  // google-veo-vertex also excluded (GOOGLE_CLOUD_PROJECT not configured).
  motion_video:      ['google-veo-flow', 'fal-ltx'],
  // hero_video: Veo-PRIMARY per warm-coalescing-island plan. Used per-beat as the
  // first-choice visual; only falls back to FLUX-still+parallax when Veo is
  // unavailable (budget out, session dead, or no clip in the Ragnar library).
  hero_video:        ['google-veo-flow', 'fal-ltx', 'local-parallax-generator'],
...
const KEY_REQUIREMENTS = {
  ...
  'google-veo-vertex':         ['GOOGLE_CLOUD_PROJECT'], // API-driven access via Vertex AI
