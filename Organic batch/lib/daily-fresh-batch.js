...
const today = String(readBootArg('date', localDateStamp()) || localDateStamp()).slice(0, 10);
...
async function runOrganicLane(opts) {
  if (!opts.organicN) {
    console.log('\n=== ORGANIC LANE skipped (organic=0) ===');
    return [];
  }
  ...
  // L114 — resilient trending fetch: a transient RSS 0 (network blip) must never
  // starve the organic lane (the recurring "organic 0/0"). Retry up to 3x with
  // backoff; if STILL empty, fall back to a curated geopolitics seed pool so the
  // lane always ships.
  let trending = [];
  for (let attempt = 1; attempt <= 3; attempt++) {
    try { trending = await fetchTrending({ topN: Math.max(opts.organicN * 8, 16) }); } catch (e) { trending = []; console.log(`  trending fetch attempt ${attempt} threw: ${(e && e.message || e).slice(0, 80)}`); }
    if (trending && trending.length) break;
    console.log(`  trending fetch attempt ${attempt} returned 0; ${attempt < 3 ? 'retrying…' : 'using fallback seed pool'}`);
    if (attempt < 3) await new Promise((r) => setTimeout(r, 2500 * attempt));
  }
  if (!trending || !trending.length) {
    const seeds = [
      'Iran nuclear talks reach a critical 60-day deadline as oil markets brace',
      'Russia escalates strikes on Ukraine cities amid stalled ceasefire diplomacy',
      'Strait of Hormuz tensions spike as naval forces mass at the chokepoint',
      'Israel and Hezbollah trade fire across the Lebanon border as truce frays',
      'China and Taiwan tensions rise after new military drills near the strait',
      'US sanctions and a fragile ceasefire test the Middle East power balance',
    ];
    trending = seeds.map((title) => ({ title, summary: title, sources: ['fallback-geopolitics'] }));
    console.log(`  [fallback] seeded ${trending.length} evergreen geopolitics topics (trending RSS was empty)`);
  }
  ...
  // L115 — FIFA lane: inject a rights-safe WC26 cinematic-data topic (Lane A, zero
  // footage). Bypasses the geopolitics niche-lock + predict gate (it's a forced
  // topic). Gated by FIFA_ORGANIC=1. NOTE: long-term FIFA should be its OWN channel
  // (don't dilute the geopolitics classification) — this is a response-test.
  if (process.env.FIFA_ORGANIC === '1') {
    try {
      const ft = await require('./fifa-schedule').generateTopic({ lane: 'organic' });
      candidates = [{ title: ft.title, summary: ft.summary, sources: ft.sources, _fifa: true }, ...candidates];
      console.log('[fifa] injected WC26 organic topic: ' + ft.title.slice(0, 64));
    } catch (e) { console.log('[fifa] organic inject failed: ' + (e && e.message || e).slice(0, 80)); }
  }
    ...
    try {
      ...
      if (r.ok) {
        if (r.pendingExternalAssets) {
          console.log(`  ✓ Prompts generated for handoff in: ${r.outDir}`);
        } else {
  ...
  for (const c of candidates.slice(0, opts.clipsN)) {
    ...
    if (!fs.existsSync(sourceFile)) {
      ...
      const dl = spawnSync(ytDlpBin, [
        ...
        '--no-progress',
        // L120 — HARD 1080p CAP. 2160p sources (16Mbps 4K) fed into the split-screen
        // filtergraph spiked ffmpeg memory and hard-crashed the 16-32GB laptop twice.
        // Output is 1080-wide: 4K source = 4x pixels, 4x RAM, ZERO quality gain.
        '-f', `bestvideo[height<=1080][height>=720][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=1080][height>=720]+bestaudio/best[height<=1080][height>=720]`,
        '--merge-output-format', 'mp4',
        '-o', path.join(sourceDir, 'source.%(ext)s'),
        c.url,
      ], { encoding: 'utf8', timeout: 30 * 60 * 1000, maxBuffer: 100 * 1024 * 1024 });
    ...
    for (let mi = 0; mi < moments.length; mi++) {
      const m = moments[mi];
      const id = `B${bIndex++}`;
      // L107+ horror routing — IShowSpeed horror VODs render FULL-FRAME (no b-roll).
      // L112 — funny TALKING podcasts (Theo Von / Kill Tony / Kai Cenat) now render
      // FULL-FRAME with ACTIVE-SPEAKER reframe (tools/speaker-reframe.py) instead of
      // the subway-surfers split. Explicit c.mode wins; CLIP_SPEAKER_REFRAME=0 reverts.
      const clipMode = c.mode
        || (c.isHorror ? 'full_frame_horror'
          : (process.env.CLIP_SPEAKER_REFRAME !== '0' ? 'full_frame_reframe' : 'split_screen'));
      clipSpecs.push({
        id,
...
async function main() {
  ...
  // LANES SIDE-BY-SIDE (user ask). Organic (FLUX/Remotion) and clip (yt-dlp/
  // ffmpeg/whisper) have different bottlenecks — organic is often network-bound
  // on FLUX/i2v while clip is CPU-bound on cutting — so running them in parallel
  // overlaps the waits and cuts wall-clock roughly in half. Promise.allSettled so
  // one lane crashing never takes down the other. SKIP_PARALLEL_LANES=1 reverts
  // to sequential if the 4GB box ever thrashes.
  let organic, clips;
  if (process.env.SKIP_PARALLEL_LANES === '1') {
    organic = await runOrganicLane(opts);
    clips = await runClipsLane(opts);
  } else {
    const [oR, cR] = await Promise.allSettled([runOrganicLane(opts), runClipsLane(opts)]);
    organic = oR.status === 'fulfilled' ? oR.value : [];
    clips = cR.status === 'fulfilled' ? cR.value : [];
    if (oR.status === 'rejected') console.log('  ⚠ organic lane threw: ' + String(oR.reason && oR.reason.message || oR.reason).slice(0, 160));
    if (cR.status === 'rejected') console.log('  ⚠ clip lane threw: ' + String(cR.reason && cR.reason.message || cR.reason).slice(0, 160));
  }
  ...
  if (opts.autoUpload && !opts.dryRun) {
    const okOrganic = organic.filter((r) => r.ok && !r.pendingExternalAssets).length;
    const okClips = clips.filter((r) => r.ok).length;
    if (okOrganic + okClips === 0) {
      console.log('\n[auto-upload] no completed renders — skipping upload chain (pending external assets or empty)');
      return;
    }
    console.log(`\n[auto-upload] firing upload chain: ${okOrganic} organic + ${okClips} clip(s), ${opts.uploadGapMin}m gap`);
    if (process.env.UPLOAD_VIA_QUEUE === '1') {
      // L116 A1 — enqueue every successful render + hand off to the persistent daemon,
      // then RETURN. The daemon (process-death-proof) drains the queue and resumes
      // through any outage/reboot, so this orchestrator process can exit safely.
      try {
        const queue = require('./upload-queue');
        let n = 0;
        const _qaMin = Number(process.env.RENDER_QA_MIN || 55);
        // Clips use a separate lower threshold — split-screen creator reactions are a
        // different format and should not be judged by the organic content rubric.
        // The Subway Surfers split-screen IS the intentional format for @ragnarautomated;
        // QA vision models label it "brainrot slop" by design. Default bypass (0) is
        // permanent. Set RENDER_QA_CLIPS_MIN >0 in .env only to re-enable screening.
        const _qaClipsMin = Number(process.env.RENDER_QA_CLIPS_MIN || 0);
        for (const r of organic.filter((x) => x.ok && !x.pendingExternalAssets)) {
          if (r.qa && r.qa.score < _qaMin) { console.log(`[qa-gate] BLOCKED organic ${r.scriptId || '?'}: score ${r.qa.score} < ${_qaMin} (${r.qa.verdict})`); continue; }
          queue.enqueue({ render: r, kind: 'organic' }); n++;
        }
        for (const r of clips.filter((x) => x.ok)) {
          if (r.qa && r.qa.score < _qaClipsMin) { console.log(`[qa-gate] BLOCKED clip ${r.id || '?'}: score ${r.qa.score} < ${_qaClipsMin}`); continue; }
          queue.enqueue({ render: r, kind: 'clip' }); n++;
        }
        // L120 — QUEUE_STAGGER_MIN (e.g. 240 = 4h cadence, organics first): rewrite
        // nextAttemptAt BEFORE the daemon spawns so it can never race ahead and post
        // everything back-to-back. Today's already-done uploads count as consumed
        // slots, so a same-day second batch continues the cadence instead of resetting it.
        if (Number(process.env.QUEUE_STAGGER_MIN)) {
          try {
            const rs = require('child_process').spawnSync(process.execPath, [path.join(ROOT, 'tools', 'restagger-queue.js'), String(process.env.QUEUE_STAGGER_MIN)], { encoding: 'utf8', windowsHide: true, timeout: 30_000 });
            console.log('[stagger] ' + String(rs.stdout || '').trim().split('\n').join('\n[stagger] '));
          } catch (e) { console.log('[stagger] failed: ' + (e && e.message || e)); }
        }
        const { spawn } = require('child_process');
        const d = spawn(process.execPath, [path.join(ROOT, 'tools', 'upload-daemon.js')], { detached: true, stdio: 'ignore', windowsHide: true });
        d.unref();
        console.log(`[auto-upload] enqueued ${n} job(s) → detached upload daemon (resumes through outage/restart)`);
      } catch (e) { console.error('[auto-upload] queue enqueue FAILED:', (e && e.message || e).slice(0, 200)); }
    } else {
      try {
