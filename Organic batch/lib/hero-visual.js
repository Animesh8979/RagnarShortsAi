 ...
 *
 * Order: Tier-2 cloud i2v (hook + climax only) → Tier-1 parallax.
 * Pexels fallback is disabled by default for organic identity quality.
 *
...
async function applyMotionCascade({ stillResult, provider, prompt, beat, scriptContext, durationSec, beatIndex, outputPath }) {
  ...
  try {
    ...
    if (i2v.shouldUseHeroI2v(beatIndex || 0, totalBeats)) {
      ...
      // L108 P4 — request first=last frame loop for hero beats so the
      // 4s motion arc closes seamlessly (Kling-style "magic loop" effect
      // viewers can't perceive a cut on). Only HF LTX-Video honors it;
      // other providers ignore the param.
      const r2 = await i2v.generateHeroBeat({ imagePath: stillResult.path, motionPrompt, durationSec, loopMode: 'first_eq_last' });
      if (r2.ok) {
        return {
          ok: true,
          path: r2.path,
  ...
  // RealMotion Tier-1.5 — Pexels stock fallback HAS BEEN PURGED.
  // The user explicitly requested "no photos bullshit" and we now strictly rely
  // on Remotion's mathematical/procedural animations when video generation fails.
  console.log(`   ℹ  Pexels stock fallback has been officially purged. Falling back directly to Ken Burns or procedural Remotion.`);
