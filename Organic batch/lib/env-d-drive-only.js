...
// FORCE-replace, not just default: .env may explicitly pin the retired
// gemini-1.5-flash (it does), so `|| default` wouldn't win. Any 1.5 ref → 2.5.
if (!process.env.GEMINI_SCRIPT_PRIMARY_MODEL || /gemini-1\.5/.test(process.env.GEMINI_SCRIPT_PRIMARY_MODEL)) {
  process.env.GEMINI_SCRIPT_PRIMARY_MODEL = 'gemini-2.5-flash';
}
if (!process.env.AESTHETIC_QA_MODEL || /gemini-1\.5/.test(process.env.AESTHETIC_QA_MODEL)) {
  process.env.AESTHETIC_QA_MODEL = 'gemini-2.5-flash';
}
