...
const DEFAULT_CREATORS = [
  ...
  { name: 'AdinRoss',        url: 'https://www.youtube.com/@AdinRoss/videos',        category: 'streamer',  clipRisk: 'chill' },
...
function listPlaylistRecent(creator, n = 12) {
  ...
  // L117 — CLIP_FOOTBALL=1: run YouTube search for football videos of IShowSpeed/MrBeast
  if (process.env.CLIP_FOOTBALL === '1' && /IShowSpeed|MrBeast/i.test(creator.name || '')) {
    const query = `ytsearch${n}:${creator.name} football`;
    let items = [];
    try {
...
async function discoverHdClips({
  ...
  // L117 — CLIP_FOOTBALL=1: restrict creators to MrBeast + IShowSpeed only
  if (process.env.CLIP_FOOTBALL === '1') {
    creators = creators.filter(c => /IShowSpeed|MrBeast/i.test(c.name || ''));
    minDurationSec = 60; // Lower from 8 minutes to 60 seconds to catch past viral funny moments
    perCreator = 30;     // Search deeper into their past videos
  }
  ...
  for (const creator of creators) {
    ...
    for (const item of recent) {
      // L117 — CLIP_FOOTBALL=1: only MrBeast/IShowSpeed football/FIFA clips (user request).
      if (process.env.CLIP_FOOTBALL === '1') {
        if (!/IShowSpeed|MrBeast/i.test(item.creator || creator.name || '')) continue;
        if (!/\b(football|soccer|fifa|world ?cup|ronaldo|cristiano|messi|mbapp|neymar|premier league|champions league|ucl|barcelona|real madrid|penalt|free ?kick|stadium|footy|haaland|world ?cup)\b/i.test(item.title || '')) continue;
      }
  ...
  if (horrorPriority && process.env.CLIP_FOOTBALL !== '1') {
  ...
  // Phase 3 T1 — filter out any source that has ever been struck/blocked
  try {
    const { filterStruck } = require('./strike-monitor');
    const before = merged.length;
    const safe = filterStruck(merged);
    if (safe.length < before) console.log(`[strike-monitor] filtered ${before - safe.length} struck source(s) from discovery`);
