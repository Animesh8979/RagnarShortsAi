...
const DOMINATION_PROMPT = `You are a SUPREME COUNCIL of four people watching a CINEMATIC 9:16 short — full-bleed Veo plates, zero box furniture, no traditional caption boxes. This is intentionally cinematic, NOT a safe-zone social template. Judge accordingly.

THE SEATS (each scores 0-100 and speaks in their own voice, 2-3 blunt sentences):
1. "editor" — veteran film/TV editor. Plate continuity between beats, color grade cohesion, whether the footage carries the story without template furniture.
2. "youtuber" — 100M-sub shorts operator. Does the cinematic look STOP the scroll? Hook in frame 1, audio mix impact, beat sync feeling, would this get 80% view-through?
3. "scroller" — bored 19-year-old. Gut reaction to full-bleed visuals — immersive or confusing? Where do you swipe?
4. "brand" — creative director. Does this look like premium ONE-channel cinematic work? Grade consistency, typography on footage readability, overall atmosphere.

DO NOT penalize: absence of safe-zone boxes, full-screen footage, minimal UI chrome, burnt-in captions over footage. These are INTENTIONAL design choices.
DO judge: plate visual quality, grade consistency, audio-visual sync, typography readability on footage, narrative flow, hook strength, emotional arc.
...
Return STRICT JSON only:
{
  ...
  "unified": <weighted 0-100: scroller 35%, youtuber 30%, editor 20%, brand 15%>,
  "shipVerdict": "ship" | "fix" | "reject",   // ship>=65, fix 40-64, reject<40
  "topFixes": ["the 3 most concrete, mechanical fixes — name the timestamp/element"],
  "bestMoment": "the single strongest moment and why"
}
Be brutal and specific. Generic praise is forbidden.`;
...
async function councilReview(videoPath, opts = {}) {
  try {
    ...
    const prompt = opts.mode === 'domination' ? DOMINATION_PROMPT : COUNCIL_PROMPT;
